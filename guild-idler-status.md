...
- **Health stat + Fallen/death mechanic** -- grew directly out of the
  ...
  - **Severity scales off each injury's own existing data** -- a new
    `healthDamagePercent` field per `InjuryDef` (edited via devtool same
    as everything else in `injuries.json`), scaled roughly with each
    injury's existing `durationHours`/`weight`. The common, low-weight
    injuries (`bruised`, `sprained_ankle`) barely dent the pool; only
    stacking the rare, high-weight ones (`dragonfire_scorch`,
    `world_ender_mark`) meaningfully drains it -- self-balancing off data
    that already exists rather than a new severity curve to hand-tune.
  - **Max Health** -- both endurance and level, same shape `injuryResist`
    already uses (`Math.sqrt(stats.endurance) * 2.0`): something like
    `100 + sqrt(endurance)*X + level*Y`, baseline high enough that a
    fresh level-1 hero isn't fragile.
  ...
  - **Regen, two rates.** Full idle regen at the guild (~1 hour to fully
    heal at baseline, finally giving the long-defined-but-unused
    `REST_TICK` constant in `items.ts` a job) plus a smaller passive
    trickle rate while a hero is actively out on a quest, so a long quest
    isn't a total recovery freeze, just slower than resting. A new 6th
    guild facility, **Infirmary** (alongside Barracks/Treasury/Workshop/
    Library/Tavern, same cost-curve shape), reduces both rates' time per
    level.
  - **Fallen state (the "death" mechanic) triggers at exactly 0 Health.**
    ...
    - **Revival, dual path reusing the exact pattern `treatInjury`
      already implements:** free after a long fixed timer (a day,
      roughly) with no action needed, or an instant paid revive whose
      gold cost scales with hero level -- same shape as `treatmentCost`
      already scaling with injury severity, same UI button pattern
      already built in `HeroesPanel`.
  ...
  **Still open:** exact constants (max-health formula coefficients,
  per-injury `healthDamagePercent` values, revival cost formula, Infirmary
  cost/growth/maxLevel/effect-per-level); whether a Fallen hero still
  occupies their guild hero slot (assumed yes -- they're down, not gone)
  or still counts toward Guild Power while Fallen; and the tombstone art
  asset itself.
