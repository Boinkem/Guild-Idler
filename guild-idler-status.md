...
- **Health stat + Fallen/death mechanic** -- grew directly out of the
  ...
  - **Regen, two rates, both scaling with the new Infirmary facility.**
    ...
    smaller passive trickle rate applies while a hero is actively out on
    a quest, so a long quest isn't a total recovery freeze, just slower
    than resting -- exact trickle fraction still open, but it should scale
    down proportionally as Infirmary levels reduce the idle time too.
  ...
  **Still open:** max-health formula coefficients (`X`/`Y` in `100 +
  sqrt(endurance)*X + level*Y`); per-injury `healthDamagePercent` values;
  the revival cost formula's actual scale factor (confirmed to scale with
  hero level, matching `treatmentCost`'s shape -- exact numbers not
  picked yet); the on-quest trickle-regen fraction relative to idle rate;
  and the tombstone art asset itself.
