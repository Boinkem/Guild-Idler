...
// Hero and pet sprite packs -- both gitignored (licensed, not
// redistributable), same convention as every other art tree here. Not
// previously served to the DevTool at all (only the game's own Vite dev
// server ever loaded them) -- added so the Pet Sprite Lab tab can show a
// real animated preview instead of asking for scale/offset numbers blind.
// Read-only from the DevTool's side: unlike ICONS_DIR/BANNERS_DIR/DECOR_DIR
// there is no upload endpoint here, since these packs are generated by
// tools/import_characters.py and tools/import_pets.py, not hand-picked
// one file at a time.
...
const SCHEMAS = {
  ...
  'pets': {
    ...
    fields: {
      ...
      requiresDlc: { type: 'string', required: false },
      // Corner-companion display corrections -- same role HeroSprite's own
      // hardcoded HERO_DISPLAY_SCALE/OFFSET tables play for hero classes,
      // except authored here as data so they're tunable without a code
      // change. Editable as plain numbers in this form, but normally set
      // from the Pet Sprite Lab tab instead (adventurer + pet side by
      // side, live animated preview, slider-driven) -- see selectPetLabTab
      // in app.js. Omitted = 1/0/0, no correction, same as before these
      // fields existed.
      displayScale: { type: 'number', required: false },
      displayOffsetX: { type: 'number', required: false },
      displayOffsetY: { type: 'number', required: false },
...
const server = http.createServer(async (req, res) => {
  ...
  // Hero and pet sprite packs, for the Pet Sprite Lab tab's live preview --
  // same guarded shape as every other art route above, but the mime map
  // also needs '.json' here (unlike the others), since each tree's own
  // manifest.json -- frame sizes and per-animation frame counts -- lives
  // inside the same folder as the .png frames and is fetched through this
  // same route, not a separate endpoint. Both trees are commonly ABSENT
  // (gitignored, licensed art the DevTool has no reason to assume is
  // installed) -- a 404 here is the normal, expected case on a fresh
  // clone, not an error condition, and the Lab tab's own fetch handles it
  // as "no art yet" rather than surfacing it as a failure.
  if (url.pathname.startsWith('/heroes-art/')) {
  ...
  if (url.pathname.startsWith('/pets-art/')) {
    const rel = decodeURIComponent(url.pathname.slice('/pets-art/'.length)).split('?')[0];
    const filePath = path.join(PETS_ART_DIR, rel);
    if (!filePath.startsWith(PETS_ART_DIR)) { res.writeHead(403); res.end(); return; }
    try {
      const body = await fs.readFile(filePath);
      const ext = path.extname(filePath).toLowerCase();
      const mime = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp', '.gif': 'image/gif', '.json': 'application/json' }[ext]
